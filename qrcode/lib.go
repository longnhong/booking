...
func Encode(content string, level RecoveryLevel, size int, colorBG string, colorFG string) ([]byte, error) {
	var q *QRCode

	q, err := New(content, level, colorBG)
...
func WriteFile(content string, level RecoveryLevel, size int, filename string, colorBG string, colorFG string) error {
	var q *QRCode
...
func New(content string, level RecoveryLevel, colorFG string) (*QRCode, error) {
	...
	//need func convert mã màu HEX sang RGBA
	//TO DO
	//
	var r, g, b, a uint8
	r, g, b = hex2rgb(colorFG)
	a = 0xff
	q := &QRCode{
		Content: content,
		...
		ForegroundColor: color.RGBA{r, g, b, a},
...
func hex2rgb(hex string) (r, g, b uint8) {
	if len(hex) > 0 && hex[0] == '#' {
		hex = hex[1:]
	}
	if rgb, err := strconv.ParseUint(string(hex), 16, 32); err == nil {
